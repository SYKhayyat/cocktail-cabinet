...
export function draw(model, context) {
  ...
  model.gems.filter((gem) => !gem.collected).forEach((gem) => { context.fillStyle = "#22d3ee"; context.beginPath(); context.arc(gem.x, gem.y, 10, 0, Math.PI * 2); context.fill(); });
  ...
  drawText(context, model.side === "runner" ? "Move with the mouse or A/D · collect falling blue gems · avoid red stars" : "Click or drag for red stars · double-click or hold for blue gems", 16, 28, 14, "#cbd5e1");
